    ...
    class Program
    {
        ...
        private static void stopService(MyService service)
        {
            ...
            if (srv.Status == ServiceControllerStatus.Running)
            {
                ...
                RunAsync(new MyService { ServiceId = service.ServiceId, TmsId = service.TmsId, displayName = srv.DisplayName, status = "Stopped", name = srv.ServiceName, MachineName = srv.MachineName, Description = "Stoppet av operatør" }).Wait();
        ...
        public static Task  sendStatusAsync(MyService service)
        {
        ...
        private static async Task StartAttempt(MyService service) {

            // var srv = new ServiceController("WebsiteStatus");
            bool test = false;
            for (int i=0;i<20;i++)
            {
                var MySrv = new ServiceController(service.name);
